...
class Article extends Component {
  ...
  render() {
   ...
   let authors = (this.props.data.hasOwnProperty('authors')) ? this.props.data.authors.map(d => { return d.name}).join(', ') : ''


    return (
      <div className='article'>
        <div className='article-title'>
          <p className='article-author'>{(authors) ? authors : 'Unknown'}, {year.length ? (' ' + year + '. ') : ''}</p>
          <a href={this.props.data.url} target='_blank' className='title-link'>
            <strong>{this.props.data.title}.</strong>
          </a>

          <span>
            <AddBoxIcon onClick={this.toggleExpand} classes={buttonClasses} className={this.state.expanded ? 'hidden' : ''}/>
            <RemoveCircleOutlinedIcon onClick={this.toggleExpand} classes={buttonClasses} className={this.state.expanded ? '' : 'hidden'}/>
          </span>
        </div>
