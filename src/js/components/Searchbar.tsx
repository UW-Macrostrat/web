...
class Searchbar extends Component {
  ...
  render() {
    ...
    let categoryResults = resultCategories.map((cat) => {
      ...
      return thisCat.map((item, h) => {
        return (
          ...
          >
            <ListItemText
              classes={{ root: "searchresult-item" }}
              primary={item.name}
    ...
    let searchResults = resultCategories.map((cat, i) => {
      return (
        <div key={`subheader-${i}`}>
          <ListSubheader classes={{ root: "searchresult-header" }}>
            {categoryTitles[cat]}
          </ListSubheader>
          {categoryResults[i]}
        </div>
    ...
    let timeFilters = this.props.filters
      .filter((f) => {
      ...
      <Paper classes={{ root: "filter-paper" }}>
        <ExpansionPanel>
          <ExpansionPanelSummary
            ...
            classes={{ root: "searchbar-filter-container" }}
          >
                ...
                return (
                  <ListItem key={i}>
                    <Tooltip title="By default only 'best' lithology matches are shown. Toggle to show all matches or best.">
                      <FormControl
                        className={d.category != "lithology" ? "hidden" : ""}
                      >
                        <FormControlLabel
                          control={
                            <Switch
                              checked={
                                d.type.substr(0, 4) === "all_" ? true : false
                              }
                              onChange={() => {
                                this.props.swapLithType(d);
                              }}
                            />
                          }
                          label={
                            d.type.substr(0, 4) === "all_"
                              ? "All matches"
                              : "Best matches"
                          }
                        />
                ...
                <div>
                  ...
                  >
                    ...
                    <ListItem>
                      <ListItemText
                        inset
                        primary="Environments (columns only)"
                      />
                    </ListItem>
                    <ListItem>
