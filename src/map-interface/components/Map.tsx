...
class Map extends Component {
  ...
  componentDidMount() {
            ...
            this.map.setFeatureState(
              {
                ...
                id: this.hoverStates[layer.layer],
              },
              { hover: false }
            );
          }
    ...
    this.map.on("click", (event) => {
      ...
      let burwellFeatures = this.map
        .queryRenderedFeatures(event.point, { layers: ["burwell_fill"] })
        .map((f) => f.properties);

      let columns = this.map
        .queryRenderedFeatures(event.point, { layers: ["column_fill"] })
        .map((f) => f.properties);
