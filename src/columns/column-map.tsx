...
class ColumnPath__ extends Component {
  render() {
    ...
    return h('path', {
      className,
      onMouseEnter() { return actions.setHovered(column); },
      onMouseLeave() { return actions.setHovered(); }
    });
  }
  componentDidMount() {
...
const ColumnPath = props=> {
  return h(MacrostratColumnConsumer, null, ({actions, hoveredColumn, helpers}) => h(ColumnPath__, {...props,actions,hoveredColumn,helpers}));
};
...
class ColumnIndexMap__ extends Component {
  ...
  render() {
    ...
    if (selection.length > 0) {
      clearSelectionButton = h(Button, {
        ...
        icon: 'graph-remove',
        onClick() { return actions.clearSelection(); }
      }, "Clear selection");
    }
    ...
    return h('div#column-index-container', [
      ...
      h(SVGComponent, {
        ...
        h('g.map-backdrop'),
        h('g.columns', columns.map(column => h(ColumnPath, {column})))
      ])
    ]);
  }
  ...
  updateProjection(prevProps, prevState){
    if (prevState != null) {
      ...
      if (this.state.width === prevWidth) { return; }
    }
  ...
  redrawPaths(sel){
    ...
    if (sel == null) { sel = this.map.selectAll('path'); }
  ...
  async componentDidMount() {
    ...
    const grat = bkg.selectAppend('path.graticule')
      .datum(graticule)
      .call(this.redrawPaths);

    const land = bkg.selectAppend('path.land')
      .datum(land50)
      .call(this.redrawPaths);

    const updateData = val=> () => {
      return land.datum(val)
        .call(this.redrawPaths);
    };
...
const ColumnIndexMap = () => {
  return h(MacrostratColumnConsumer, null, ({columns, actions, selection}) => h(ColumnIndexMap__, {columns, actions, selection}));
};
