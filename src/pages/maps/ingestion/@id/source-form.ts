...
import ProgressPopover, { ProgressPopoverProps } from "~/pages/maps/ingestion/@id/components/progress-popover/progress-popover";
...
const postIngestProcessObject = async (ingestProcessId: number, files: FileList) => {
  ...
  const response = await fetch(`${ingestPrefix}/ingest-process/${ingestProcessId}/objects`, {
    method: "POST",
    body: data
  })
...
const SourceForm = ({onSubmit, source, children}: {onSubmit: (e) => void, source?: Source, children?: ReactNode}) => {
...
const AddSourceForm = () => {
  ...
  const [progress, setProgress] = useState<ProgressPopoverProps | undefined>(undefined)
