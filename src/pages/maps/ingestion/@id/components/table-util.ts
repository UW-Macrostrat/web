...
export const applyTableUpdate = (data: any[], tableUpdate: TableUpdate) => {
  ...
  for (const [rowIndex, row] of data.entries()) {
    for (const columnName of Object.keys(row)) {
      appliedData[rowIndex][columnName] = tableUpdate.applyToCell(
        appliedData[rowIndex][columnName],
        row,
        columnName
      );
    }
...
export const getTableUpdate = (
  ...
  if (pointInTimeDataParameters?.group != undefined) {
    pointInTimeDataParameters.filter[pointInTimeDataParameters?.group] =
      new Filter(
        pointInTimeDataParameters?.group,
        "eq",
        data[rowIndex][pointInTimeDataParameters?.group]
      );
  } else {
    pointInTimeDataParameters.filter["_pkid"] = new Filter(
      "_pkid",
      "eq",
      data[rowIndex]["_pkid"]
    );
  ...
  const execute = async () =>
    submitChange(url, value, [columnName], pointInTimeDataParameters.filter);
  ...
  return {
    description:
      ...
      value +
      " for " +
      JSON.stringify(pointInTimeDataParameters.filter),
    execute: execute,
    applyToCell: apply,
  } as TableUpdate;
};
