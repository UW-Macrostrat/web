...
function DisplaySettingsPanel() {
  ...
  return h("div.display-settings-panel", [
    ...
    h("h3.section", "Columns"),
    h(ColumnOptions),
    h.if(edit)([h("h3.section", "Editing"), h(EditingOptions)]),
  ]);
}

/** Which of the generated and derived values the tables show. The record —
 * names, positions, intervals and proportions, descriptors — is always there;
 * these are the values that follow from it. */
function ColumnOptions() {
  const measured = useAtomValue(positionAxisAtom) != null;
  return h([
    h(SwitchControl, {
      label: "Identifiers",
      valueAtom: showIdentifiersAtom,
    }),
    h(
      "p.setting-hint",
      "Unit, section and boundary ids: generated, never edited."
    ),
    h(SwitchControl, {
      label: "Absolute ages",
      valueAtom: showAgesAtom,
    }),
    h(
      "p.setting-hint",
      "Derived from each boundary's interval and proportion through the age model. Interpolated ages are drawn dimmed."
    ),
    h(SwitchControl, {
      label: "Mark repeated attributes",
      valueAtom: dimFilledValuesAtom,
      disabled: !measured,
    }),
    h(
      "p.setting-hint",
      measured
        ? "A lithology, environment or name that repeats the unit below it (above it, on a depth column) is drawn dimmed behind an arrow — the ingestion sheet fills those in."
        : "Attributes fill along a measured axis; this column has none."
    ),
  ]);
}
