...
import type { BoundaryKind, IntervalDef } from "./boundaries";
import {
  boundaryTolerance,
  findIntervalByName,
  unitBoundary,
...
/** An interval named in a cell must be one Macrostrat knows: the format takes
 * names, and an unknown one calibrates nothing. Silent while the definitions
 * are still loading, so nothing is red for want of a fetch. */
export function validateIntervalName(
  intervals: Map<number, IntervalDef> | null | undefined
) {
  return (value: any): CellValidation | null => {
    if (value == null || value === "") return null;
    if (intervals == null || intervals.size === 0) return null;
    if (typeof value === "object") return null;
    if (findIntervalByName(intervals, String(value)) != null) return null;
    return { severity: "error", message: "Not a known interval" };
  };
}
...
export function unitIssues(
  ...
  positionAxis: ColumnAxisType | null = null
): UnitIssue[] {
  const issues: UnitIssue[] = [];
  const [bottomField, topField] = BOUNDARY_FIELDS[kind];

  for (const unit of units) {
    if (unitRange(unit, kind) == null) {
      // Only a measured column is expected to carry positions, so a missing
      // one is worth saying only where the column is drawn on that index.
      issues.push({
    ...
    issues.push(...reversedBoundaryIssues(unit, kind, positionAxis));
  }
  issues.push(...chronoOrderIssues(units));
...
/** A unit whose top and base read backwards: on an age index the base must
 * be the older bound; on a measured one the top must lie in the direction
 * the column is measured (upwards for height, downwards for depth). */
function reversedBoundaryIssues(
  unit: UnitLong,
  kind: BoundaryKind,
  ...
  let message = "";
  if (kind === "chrono") {
    reversed = bottom < top - boundaryTolerance(kind);
    message = "The base is younger than the top.";
  } else if (positionAxis === ColumnAxisType.DEPTH) {
    reversed = top > bottom + boundaryTolerance(kind);
    message = "On a depth column the top is the shallower position.";
  } else {
    reversed = top < bottom - boundaryTolerance(kind);
    message = "On a height column the top is the higher position.";
  }
  if (!reversed) return [];
  return [bottomField, topField].map((field) => ({
    unit_id: unit.unit_id,
    field,
...
/** The format's own rules for the chronostratigraphic fields: `b_prop` and
 * `t_prop` lie in 0–1 (a cell validator says so) and, when both bounds sit
 * in one interval, the base's proportion is below the top's. */
function chronoOrderIssues(units: UnitLong[]): UnitIssue[] {
  const issues: UnitIssue[] = [];
  for (const unit of units) {
    const { b_int_id, t_int_id, b_prop, t_prop } = unit;
    if (b_int_id == null || t_int_id == null || b_int_id !== t_int_id) continue;
    if (b_prop == null || t_prop == null) continue;
    if (Number(b_prop) <= Number(t_prop)) continue;
    for (const field of ["b_prop", "t_prop"]) {
      issues.push({
        unit_id: unit.unit_id,
        field,
        severity: "error",
        message:
          "Within one interval the base's proportion is below the top's.",
      });
    }
  }
  return issues;
