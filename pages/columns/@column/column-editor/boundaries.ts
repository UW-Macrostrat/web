...
export function readBoundaryEdit(
  ...
  // An interval picked with a position in it — what the interval editor
  // hands back: the interval is known, and the proportion comes with it.
  if (isIntervalPosition(rawValue)) {
    if (rawValue.int_id == null) return null;
    const def = intervals?.get(rawValue.int_id) ?? null;
    const prop = rawValue.prop ?? unit[`${prefix}_prop`] ?? null;
    const values: BoundaryValues = {
      int_id: rawValue.int_id,
      int_name: def?.name ?? rawValue.int_name ?? undefined,
    };
    if (prop != null) values.prop = prop;
...
/** A chronostratigraphic position as the interval editor hands it back. */
export interface IntervalPositionValue {
  int_id: number | null;
  int_name?: string | null;
  prop?: number | null;
}

function isIntervalPosition(value: unknown): value is IntervalPositionValue {
  return (
    value != null && typeof value === "object" && "int_id" in (value as any)
  );
}
