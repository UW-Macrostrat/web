...
/** Whether the generated identifiers (`unit_id`, `section_id`) are shown.
 * They are never written, and most of the time they are noise beside the
 * fields being edited — but they are what a row *is*, so they can be
 * brought back. */
export const showIdentifiersAtom: PrimitiveAtom<boolean> = atom(false);

/** Whether the absolute ages (`b_age`, `t_age`, a surface's model age) are
 * shown. They are derived — the record is an interval and a proportion, or a
 * measured position, and the age follows through the age model — so the unit
 * sheets hide them by default and show them on request, dimmed. */
export const showAgesAtom: PrimitiveAtom<boolean> = atom(false);

/** Whether an attribute that merely restates the unit below it (above it, on
 * a depth column) is drawn dimmed with an arrow, the way the ingestion
 * spreadsheet lets it be left blank and "filled up". The value is still the
 * unit's own; only the drawing changes. */
export const dimFilledValuesAtom: PrimitiveAtom<boolean> = atom(true);
