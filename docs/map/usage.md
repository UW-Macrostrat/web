...
Macrostrat's [map interface](/map) is the primary tool to interact with our
harmonized multiscale geologic map. This guide tours its capabilities; the
[changelog](changelog.md) records when each arrived.
...
and loaded on demand. This allows easy sharing of specific geologic features
with others. The _Copy link_ button at the top of the info panel allows easy
...
levels. This display is intuitive and accurate, but it cannot show all
continents at once. To switch back to a Mercator projection, click the _map_
button in the bottom right of the map interface.
...
_3D_ button that appears in the bottom right corner of the map. Drag while
right-clicking the mouse to adjust the view orientation. Click the compass at
...
names, lithology types, and time intervals.
...
relevant to specific units and map areas.
...
fossil collections, and satellite imagery can be overlaid on the geologic map.
...
profile between them. This capability is a placeholder for in-development 3D
