...
- **`zustand-computed` is pinned to 2.1.0** (`resolutions` in `package.json`). 2.1.1 mutates the store's state object in place inside `setState`, so zustand's `Object.is` guard never fires listeners — components stop re-rendering on store updates (the correlation map's columns appearing only after a layout toggle). The library is moving those stores to jotai atoms; drop the pin once no `@macrostrat/*` dependency pulls `zustand-computed`.
