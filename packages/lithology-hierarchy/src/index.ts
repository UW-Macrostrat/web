...
export default function MacrostratLithologyHierarchy() {
  ...
  return h("div.flex.row", [
    h(
      ...
      h(Example, { width: 800, height: 2000, data: nestLiths(liths) })
    ),
  ]);
}
...
function nestLiths(liths: Lith[]): TreeNode {
  const root: TreeNodeMap = { name: "All lithologies", children: new Map() };
