    ...
    */
    ...
    Utilities.fetchMapData(
      ...
      (mapError, data, refs) => {
        ...
        Utilities.fetchData(
          `defs/strat_name_concepts?${type}=${id}`,
          (conceptError, conceptData) => {
            if (conceptError || !conceptData.success) {
              return this.setState(this._resetState());
            }

            if (conceptData.success && conceptData.success.refs) {
              parsedRefs = parsedRefs.concat(
                Object.keys(conceptData.success.refs).map((d) => {
                  return conceptData.success.refs[d];
                })
              );
            }
            ...
            Utilities.fetchData(
              "defs/strat_names?" + params,
              (stratNameConceptError, stratNameConceptData) => {
                ...
                this.setState({
                  ...
                  strat_names: stratNameConceptData.success.data.sort(
                    (a, b) => {
      ...
      <div className="page-content">
        ...
        <div className={this.state.mapData.features.length ? "" : "hidden"}>
          ...
          <div className="row chart-row">
            <div className={chartClass}>{lithChart}</div>
            <div className={chartClass}>{environChart}</div>
            <div className={chartClass}>{econChart}</div>
          </div>
        ...
        <div
          className={
            this.state.concept.concept_id ? "row concept-info" : "hidden"
          }
        >
          <div className="col-sm-12">
            <div className="concept-group-title">
              <h3>{this.state.concept.name}</h3>
              <span className="concept-ref">
                via{" "}
                <a
                  href={this.state.concept.url}
                  target="_blank"
                  className="normalize-link"
                >
                  {this.state.concept.author}
                </a>
              </span>
            </div>
          </div>
          <div className="col-sm-4">
            <div className="concept-group-attributes">
              {this.state.concept.province ? (
                <LongText title="Province" text={this.state.concept.province} />
              ) : (
                ""
              )}
              {this.state.concept.geologic_age ? (
                <LongText title="Age" text={this.state.concept.geologic_age} />
              ) : (
                ""
              )}
              {this.state.concept.other ? (
                <LongText title="Notes" text={this.state.concept.other} />
              ) : (
                ""
              )}
              {this.state.concept.usage_notes ? (
                <LongText title="Usage" text={this.state.concept.usage_notes} />
              ) : (
                ""
              )}
            </div>
