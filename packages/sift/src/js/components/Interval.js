...
class Interval extends React.Component {
  ...
  _update(id) {
    ...
    Utilities.fetchMapData(`columns?int_id=${id}&response=long`, (error, data) => {
      Utilities.fetchData(`defs/intervals?int_id=${id}`, (intError, intData) => {
        if (error || intError || !data.features.length) {
    ...
    return (
      <div>
        <div className='page-title'>
          <a href={'#/interval/' + this.state.target.int_id}>{this.state.target.name}</a>
        </div>
